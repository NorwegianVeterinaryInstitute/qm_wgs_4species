...
total_df2 <- total_df %>%
  ...
  mutate_at(vars(qnrA1,qnrB19,qnrB6,qnrB60,qnrS1,qnrS2,qnrS4),
            funs(as.numeric(.))) %>%
  mutate(qnr = ifelse(rowSums(.[,3:9]) == 0, 0, 1)) %>%
  ...
  select(-c(qnr_n,gyrA_D87G_n,gyrA_D87H_n,
...
ggplot(total_df2, aes(reorder(gene, type), factor(CIP), fill = perc))+
  ...
  annotate("rect", xmin = 0, xmax = 6.5, ymin = -0.5, ymax = 0.2, fill = "grey95", color = "black")+
  annotate("rect", xmin = 6.5, xmax = 12.5, ymin = -0.5, ymax = 0.2, fill = "grey95", color = "black")+
  annotate("rect", xmin = 12.5, xmax = 19.5, ymin = -0.5, ymax = 0.2, fill = "grey95", color = "black")+
  annotate("text", x = 3.25, y = -0.05, label = "gyrA", size = 2.5)+
  annotate("text", x = 9.5, y = -0.05, label = "parC", size = 2.5)+
  annotate("text", x = 16, y = -0.05, label = "parE", size = 2.5)+
