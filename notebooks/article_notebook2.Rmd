...
## Percent mutations in each intrinsic gene per animal species
```{r}
mut_report %>%
  select(-nt_change) %>%
  left_join(isolate_data[,c("id","species")], by = "id") %>%
  gather(gene, value, -c(id, species)) %>%
  group_by(gene, species, value) %>%
  dplyr::count() %>%
  ungroup() %>%
  mutate(value = ifelse(value == 1, "Present", "Absent")) %>%
  spread(value, n, fill = 0) %>%
...
mut_filtered %>%
  ...
  mutate(Total = case_when(
    gene == "marA" ~
      length(which(mut_report[, "marA"] == 1)),
    gene == "marR" ~
      length(which(mut_report[, "marR"] == 1)),
    gene == "rpoB" ~
      length(which(mut_report[, "rpoB"] == 1)),
    gene == "soxR" ~
      length(which(mut_report[, "soxR"] == 1))),
         percent = case_when(
    gene == "marA" ~ 
      round(n / Total*100, 1),
    gene == "marR" ~ 
...
total_mechanisms <- mut_report %>%
  ...
  mutate_at(vars(qnrA1,qnrB19,qnrB6,qnrB60,qnrS1,qnrS2,qnrS4),
            funs(as.numeric(.))) %>%
  mutate(qnr = ifelse(rowSums(.[,13:18]) == 0, 0, 1)) %>%
...
total_report <- mut_report %>%
  ...
  mutate_at(vars(-id),
            funs(as.numeric(.))) %>%
  mutate(qnr = ifelse(rowSums(.[,12:18]) > 0, 1, 0)) %>%
  select(-c(qnrA1,qnrB19,qnrB6,qnrB60,qnrS1,qnrS2,qnrS4)) %>%
  gather(gene, value, -id) %>%
  mutate(present = ifelse(value == 1, gene, NA),
         ref = 1:n()) %>%
...
dot_plot <- ggplot(dot_report, aes(reorder(present, entries), gene, fill = gene))+
  geom_point(size = 2)+
  scale_y_discrete(limits = c("gyrA","parC","parE","marA","marR","rpoB","soxR","qepA4","qnr"))+
  guides(fill = FALSE)+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size = 8),
        axis.title = element_blank(),
